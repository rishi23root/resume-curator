...
WorkingDirectory=$(pwd)
Environment="PATH=$(pwd)/env/bin"
ExecStart=su $USER -c '$(pwd)/env/bin/gunicorn --workers $workers --bind 0.0.0.0:$port wsgi:app --access-logfile $(pwd)/logs/access.log --error-logfile $(pwd)/logs/error.log --capture-output --log-level debug'
